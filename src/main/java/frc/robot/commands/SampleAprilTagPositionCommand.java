...
public class SampleAprilTagPositionCommand extends A05DriveCommand {
    ...
    private final double yawOffset = 9.5, areaOffset = 4.0;

    private final double yawThreshold = 0.015, areaThreshold = 0.05;
    ...
    public void execute() {
        ...
        if (!alignedWithAprilTag) {
            ...
            m_photonSubsystem.calcAreaOffsetAverage(m_photonSubsystem.lastTargetFrame, 0.0, 7.0);
            ...
            if(m_photonSubsystem.lastTargetFrame.hasTargets()) {
                ...
                m_conditionedDirection.atan2(m_photonSubsystem.getYawOffsetAverage(-30.0, 30.0, yawOffset),
                        -m_photonSubsystem.getAreaOffsetAverage(0.0, 7.0, areaOffset));
                ...
                double speedDistance = Utl.length(Math.pow(Math.abs(m_photonSubsystem.getYawOffsetAverage(-30.0, 30.0, yawOffset)), speedSmoothingMultiplier),
                        Math.pow(Math.abs(m_photonSubsystem.getAreaOffsetAverage(0.0, 7.0, areaOffset)), speedSmoothingMultiplier));
